...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    private final SimpleJdbcInsert incertMeal;

    @Autowired
    public JdbcMealRepositoryImpl(DataSource dataSource, JdbcTemplate jdbcTemplate, NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
        this.incertMeal = new SimpleJdbcInsert(dataSource)
                .withTableName("meals")
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            Number newKey = incertMeal.executeAndReturnKey(map);
            meal.setId(newKey.intValue());
        } else if (namedParameterJdbcTemplate.update("update meals set datetime=:datei=time, description=:description," +
                "calories=:calories, userid=:userid", map) == 0) {
            return null;
        }
        return meal;
    }
